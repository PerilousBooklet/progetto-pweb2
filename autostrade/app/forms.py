...
# All'interno di queste classi, sono contenuti tutti i campi
# che genereranno le form. Ogni variabile al loro interno
# indica un campo e le sue proprietà


# Form utilizzata per la ricerca nella tabella dei comuni
class ComuneForm(forms.Form):
    ...
    provincia = forms.CharField(
        label="Provincia",
        widget=forms.Select(choices=customlib.getProvincieUnique()),
        required=False,
    )
...
# Form utilizzata per il modale di modifica dei comuni
class ComuneModalEditForm(forms.Form):
    ...
    provinciaModalEdit = forms.CharField(
        label="Provincia",
        widget=forms.Select(choices=customlib.getProvincieUnique()),
        required=False,
    )
...
# Form utilizzata per il modale per la cancellazione dei comuni
class ComuneModalDeleteForm(forms.Form):
    ...
    provinciaModalDelete = forms.CharField(
        label="",
        widget=forms.Select(choices=customlib.getProvincieUnique()),
        required=False,
    )
...
# Form utilizzata per il modale per l'inserimento dei comuni
class ComuneModalInsertForm(forms.Form):
    codiceModalInsert = forms.CharField(
        label="Codice comune", max_length=100, required=False
    )
    codiceModalInsert.widget.attrs.update({"class": "form-control"})

    provinciaModalInsert = forms.CharField(
        label="Provincia",
        widget=forms.Select(choices=customlib.getProvincieUnique()),
        required=False,
    )
    ...
    nomeModalInsert = forms.CharField(
        label="Nome comune", max_length=100, required=False
    )
    nomeModalInsert.widget.attrs.update({"class": "form-control"})


# Form utilizzata per la ricerca nella tabella delle autostrade
...
# Form utilizzata per la ricerca nella tabella dei caselli
class CaselloForm(forms.Form):
    ...
    cod_naz = forms.CharField(
        label="Codice autostrada",
        max_length=100,
        widget=forms.Select(choices=customlib.getAutostradeUnique()),
        required=False,
    )
    ...
    x.widget.attrs.update(
        {"class": "form-control", "type": "number", "step": "0.000001"}
    )

    y = forms.CharField(label="Coordinata Y", required=False)
    y.widget.attrs.update(
        {"class": "form-control", "type": "number", "step": "0.000001"}
    )

    is_automatico = forms.ChoiceField(
        label="Tipo casello",
        widget=forms.RadioSelect,
        choices=opzioniAutomatico,
        required=False,
    )
    is_automatico.widget.attrs.update(
        {"class": "form-check-input", "onclick": "blockDate()"}
    )

    data_automazione = forms.DateField(
        label="Data automazione",
